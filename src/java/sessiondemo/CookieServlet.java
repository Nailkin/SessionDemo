...
public class CookieServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if(userCookies != null && userCookies.length > 0) {
                out.println("User cookies:");
                        for(Cookie c: userCookies){
                            ...
                            out.println("expire time = " + c.getMaxAge() + " seconds <br/>");
                            
                            if(c.getName().equals("lastVisit")) {
                                lastVisit = Long.parseLong(c.getValue());
                                out.println("your last visit was on " + new java.util.Date(lastVisit));                               
            ...
            Date now = new Date();
            Cookie lastVisitCookie = new Cookie("lastVisit","" + now.getTime());
